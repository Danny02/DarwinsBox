...
public final class ObjMaterial implements Externalizable, Cloneable
{
     ...
     */
    private byte illum;
    private float[] ambient = new float[3];//ka
    private float[] diffuse = new float[3];//kd
    private float[] sepcular = new float[]{0.2f, 0.2f, 0.2f};//ks
    private float[] emission = new float[3];//ke
    ...
    public GameMaterial creatGameMaterial()
    {
        String nt = getNormalTex();
        if (nt == null) {
            nt = getBumbTex();
        }
        return new GameMaterial(trimToThree(getDiffuse()),
                trimToThree(getAmbient()), trimToThree(getSepcular()),
