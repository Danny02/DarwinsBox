 ...
 */
public class Material implements Externalizable, Cloneable
{
    /**
     * Ns exponent
     ...
     * dheinrich.own.engineween 1.0 (fully opaque) and the
    ...
    public void setSepcular(float[] sepcular) {
        if (this.sepcular == null)
            this.sepcular = new float[4];
        int min = min(4, sepcular.length);
        int i;
        for (i = 0; i < min; ++i)
    ...
    public Material clone() {
        return new Material(diffuse.clone(), ambient.clone(), emission.clone(), sepcular.
                clone(),
